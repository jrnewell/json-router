    ...
    return function(httpReq, httpRes, next) {
      ...
      if (!_.isArray(requestList)) {
        if (_.isObject(requestList)) {
          var keys = _.keys(requestList);
          // do we have a single request or literal notation?
          if (!requestList.name) {
            requestList = _.transform(requestList, function(result, req, key) {
              req.requestId = key;
              if (!req.name) req.name = key;
              result.push(req);
            }, []);
          }
          else {
