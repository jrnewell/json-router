  ...
  middleware: function(opts, callback) {
    ...
    return function(req, res, next) {
      ...
      var routeRequest = function(request, callback) {
        // skip request
        if (request._skip) {
          reqUtils.updateStatus(request, "cancelled");
        ...
        if (req.dependsOn && req._parent) {
          ...
          if (req._parent.result) context.parentResult = req._parent.result;
        }
        ...
        reqUtils.updateStatus(request, "running");
        reqMapping(context, reqArgs, function(err, result) {
          // attach result object
          reqUtils.updateStatus(request, "finished");
